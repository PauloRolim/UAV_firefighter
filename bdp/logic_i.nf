...
THEORY ListSubstitutionX IS
  ...
  List_Substitution(Implementation(logic_i),SM_SimBCMonitor)==(IF SM_SimBCMonitor_state = INIT THEN SM_SimBCMonitor_state:=EXEC_1 ELSIF SM_SimBCMonitor_state = EXEC_1 THEN IF i_MissionStart = IO_OFF THEN SM_SimBCMonitor_state:=EXEC_1 END;IF i_MissionStart = IO_ON THEN var_P_1 <-- get_ms_tick;SM_SimBCMonitor_state:=EXEC_2 END ELSIF SM_SimBCMonitor_state = EXEC_2 THEN var_read_1:=0;VAR guard_1,guard_2,guard_3,guard_4,guard_5,guard_6,guard_7,guard_8,guard_9,guard_10,local_since,local_read,predicate_1,predicate_2,predicate_3,predicate_4,predicate_5,predicate_6,predicate_7,predicate_8,predicate_9,predicate_10,predicate_11,predicate_12,predicate_13,predicate_14,predicate_15,predicate_16 IN guard_1: (guard_1: BOOL);guard_2: (guard_2: BOOL);guard_3: (guard_3: BOOL);guard_4: (guard_4: BOOL);guard_5: (guard_5: BOOL);guard_6: (guard_6: BOOL);guard_7: (guard_7: BOOL);guard_8: (guard_8: BOOL);guard_9: (guard_9: BOOL);guard_10: (guard_10: BOOL);local_since: (local_since: uint32_t);local_read: (local_read: uint32_t);predicate_1: (predicate_1: BOOL);predicate_2: (predicate_2: BOOL);predicate_3: (predicate_3: BOOL);predicate_4: (predicate_4: BOOL);predicate_5: (predicate_5: BOOL);predicate_6: (predicate_6: BOOL);predicate_7: (predicate_7: BOOL);predicate_8: (predicate_8: BOOL);predicate_9: (predicate_9: BOOL);predicate_10: (predicate_10: BOOL);predicate_11: (predicate_11: BOOL);predicate_12: (predicate_12: BOOL);predicate_13: (predicate_13: BOOL);predicate_14: (predicate_14: BOOL);predicate_15: (predicate_15: BOOL);predicate_16: (predicate_16: BOOL);predicate_1:=bool(i_CommsLink = IO_ON);predicate_2:=bool(i_HighBattery = IO_ON);guard_1 <-- land(predicate_1,predicate_2);predicate_3:=bool(i_CommsLink = IO_OFF);predicate_4:=bool(i_HighBattery = IO_OFF);guard_2 <-- lor(predicate_3,predicate_4);local_since <-- since(var_P_1);guard_3:=bool(local_since<=TIMEOUT);guard_4:=bool(TIMEOUT<local_since);local_read:=0;var_currbit_2:=bool(i_CommsLink = IO_ON);predicate_5 <-- validBitPattern(PATTERN,local_read,var_currbit_2);predicate_6 <-- lnot(predicate_5);var_highBattery_3:=bool(i_HighBattery = IO_ON);predicate_7:=var_highBattery_3;guard_5 <-- land(predicate_6,predicate_7);local_read:=0;var_currbit_2:=bool(i_CommsLink = IO_ON);predicate_8 <-- validBitPattern(PATTERN,local_read,var_currbit_2);var_highBattery_3:=bool(i_HighBattery = IO_ON);predicate_9:=var_highBattery_3;guard_6 <-- land(predicate_8,predicate_9);predicate_10:=bool(TIMEOUT<local_since);predicate_11:=bool(i_HighBattery = IO_OFF);predicate_12 <-- land(predicate_10,predicate_11);guard_7 <-- land(predicate_9,predicate_12);var_highBattery_3:=bool(i_HighBattery = IO_OFF);predicate_13:=var_highBattery_3;guard_8:=predicate_13;predicate_14:=bool(i_CommsLink = IO_ON);predicate_15:=bool(i_HighBattery = IO_ON);guard_9 <-- land(predicate_1,predicate_2);var_highBattery_3:=bool(i_HighBattery = IO_OFF);predicate_16:=var_highBattery_3;guard_10:=predicate_16;IF guard_1 = TRUE THEN IF guard_5 = TRUE THEN IF guard_3 = TRUE THEN o_Proceed:=IO_ON;SM_SimBCMonitor_state:=EXEC_2 ELSIF guard_4 = TRUE THEN o_Proceed:=IO_OFF;SM_SimBCMonitor_state:=EXEC_2 END ELSIF guard_6 = TRUE THEN IF guard_7 = TRUE THEN skip ELSIF guard_8 = TRUE THEN SM_SimBCMonitor_state:=EXEC_3 ELSIF guard_9 = TRUE THEN IF guard_5 = TRUE THEN IF guard_3 = TRUE THEN o_Proceed:=IO_ON;SM_SimBCMonitor_state:=EXEC_2 ELSIF guard_4 = TRUE THEN o_Proceed:=IO_OFF;SM_SimBCMonitor_state:=EXEC_2 END ELSIF guard_6 = TRUE THEN skip ELSIF guard_10 = TRUE THEN SM_SimBCMonitor_state:=EXEC_3 END END ELSIF guard_10 = TRUE THEN SM_SimBCMonitor_state:=EXEC_3 END ELSIF guard_2 = TRUE THEN IF guard_3 = TRUE THEN o_Proceed:=IO_ON;SM_SimBCMonitor_state:=EXEC_2 ELSIF guard_4 = TRUE THEN o_Proceed:=IO_OFF;SM_SimBCMonitor_state:=EXEC_2 END END END ELSIF SM_SimBCMonitor_state = EXEC_3 THEN o_Proceed:=IO_OFF ELSE skip END;cycle_state:=st_WRITE_OUTPUTS);
  List_Substitution(Implementation(logic_i),validBitPattern)==(valid: (valid: BOOL);IF cc = TRUE THEN var_patternBit_1:=FALSE;var_patternBit_2:=pp;valid:=TRUE;var_read_1:=add_uint32(rr,1) ELSE var_patternBit_1:=TRUE;var_patternBit_2:=FALSE;valid:=FALSE END);
